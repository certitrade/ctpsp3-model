	...
	export async function upgrade(
		...
		if (key == undefined)
			...
			if (typeof key.option.card == "string" || inner) {
				const unpacked: authly.Payload | undefined =
					typeof key.option.card == "string"
						? result.aud == "public"
			...
			if (result && key.option.email)
				result = model.Merchant.Configuration.Email.is(key.option.email)
					? { ...result, email: key.option.email }
					: undefined
			if (result && key.option.mash)
				result = model.Merchant.Configuration.Mash.is(key.option.mash)
